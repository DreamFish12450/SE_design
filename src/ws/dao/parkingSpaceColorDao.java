...
import com.beans.Staff;  //引用一个车位的状态信息

public class parkingSpaceColorDao {
	public bool upparkingSpaceColor(String jobNum,Users u){
		Connection conn=null;
		Staff s=null;
		try {
			conn = JdbcUtil.getConnection(u.getUserName(), u.getPwd()); /*通过User帐号与数据库连接*/
			PreparedStatement ps=conn.prepareStatement("select * from Staff where Job_number=?"); /*创建预处理对象，并进行数据库查询*/
